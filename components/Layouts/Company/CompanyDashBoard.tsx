...
function CompanyDashBoard({ children }: { children: JSX.Element }) {
    ...
    >
      <Stack spacing={3}>
        ...
        <List sx={style} component="nav" aria-label="mailbox folders">
          {items.map((item) => (
            <Link href={`/company${item.id}`} passHref key={item.id}>
              <ListItem
                sx={{ borderRadius: 5 }}
                button
                selected={!!match(`/company${item.id}`)}
              >
                <ListItemAvatar
                  sx={{
                    color: match(`/company${item.id}`) ? "blue" : "#9e9e9e",
                  }}
                >
                  {item.avatar}
                </ListItemAvatar>
