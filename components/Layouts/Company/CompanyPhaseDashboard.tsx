...
function CompanyPhaseDashboard({ children }: { children: JSX.Element }) {
    ...
    >
      <Stack spacing={3}>
        ...
        <List sx={style} component="nav" aria-label="mailbox folders">
          {items.map((item) => (
            <Link href={`/company/rc/${rcId}${item.id}`} passHref key={item.id}>
              <ListItem
                sx={{ borderRadius: 5 }}
                button
                selected={!!match(`/company/rc/${rcId}${item.id}`)}
              >
                <ListItemAvatar
                  sx={{
                    color: match(`/company/rc/${rcId}${item.id}`)
                      ? "blue"
                      : "#9e9e9e",
                  }}
                >
